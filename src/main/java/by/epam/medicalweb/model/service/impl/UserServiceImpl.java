...
public class UserServiceImpl implements UserService {
   ...
   */
  @Override
  public User buildBasicUser(Map<String, String> data){
    User basicUser;
  ...
  public boolean changePassword(long userId, Map<String, String> data)
      throws ServiceException {
    String oldPassword = PasswordEncoder.encryptPass(data.get(OLD_PASSWORD));
    EntityTransaction entityTransaction = new EntityTransaction();
    UserDaoImpl userDao = new UserDaoImpl();
    entityTransaction.beginTransaction(userDao);
    boolean result = false;
    try {
      boolean userIsExist = userDao.findUserByIdAndPassword(userId, oldPassword);
      if (userIsExist) {
        String newPassword = data.get(NEW_PASSWORD);
        String passwordConfirm = data.get(NEW_PASSWORD_CONFIRM);
        String passCheckResult = ValidatorImpl.getInstance().isCorrectPassword(newPassword)
            ? ((newPassword.equals(passwordConfirm)) ? "true" : PASSWORD_MISMATCH)
            : INVALID_NEW_PASSWORD;
        result = Boolean.parseBoolean(passCheckResult);
        if (result) {
          String newEncryptedPassword = PasswordEncoder.encryptPass(newPassword);
          result = userDao.updateUserPasswordById(userId, newEncryptedPassword);
        } else {
          data.clear();
          data.put(PASSWORD_CHANGE_RESULT, passCheckResult);
          logger.log(Level.DEBUG, "New password mismatch or invalid");
        }
      } else {
        data.put(OLD_PASSWORD, INCORRECT_OLD_PASSWORD);
        logger.log(Level.DEBUG, "Old password is incorrect");
      }

    } catch (DaoException e) {
      entityTransaction.rollback();
      logger.log(Level.ERROR, "User password wasn't changed", e);
      throw new ServiceException("User password wasn't changed", e);
    }
    finally {
      entityTransaction.endTransaction();
    }
